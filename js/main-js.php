...
function showweek(week, option)
{
	...
	if(isNaN(week))
	{
		notify(<?php echo"'".__('Invalid week number')."'"; ?>, 4);
	}
	else
	{
		if(week < 1)
...
function page_load(page)
{
	...
	setTimeout(function()
	{
		...
		{
			notify(<?php echo"'".__('Loading...')."'"; ?>, 300);
		}
	}, 500);
...
function page_loaded(page)
{
	...
	$.get('main.php?day_number', function(data)
	{
		...
		{
			notify(<?php echo"'".__('Day have changed. Refreshing...')."'"; ?>, '300');
	...
	setTimeout(function()
	{
		if($('#notification_inner_cell_div').is(':visible') && $('#notification_inner_cell_div').html() == <?php echo"'".__('Loading...')."'"; ?>)
		{
			notify();
		}
	}, 1000);
	...
	if(page == 'about')
	{
		$('#about_latest_version_p').html(<?php echo "'".'<img src="img/loading.gif" alt="Loading">' .__('Getting latest version...')."'"; ?>);
...
function login()
{
	...
	$('#login_message_p').html(<?php echo "'".'<img src="img/loading.gif" alt="Loading">'. __('Logging in...')."'"; ?>).slideDown('fast');
	...
	$.post('login.php?login', { user_email: user_email, user_password: user_password, user_remember: user_remember }, function(data)
	{
		...
		else
		{
			if(data == '')
			{
				$('#login_message_p').html(<?php echo "'".'<span class="error_span">'.__('Wrong email and/or password').'</span>'."'"; ?>);
...
function create_user()
{
	...
	{
		$('#new_user_message_p').html(<?php echo "'".'<span class="error_span">'.__('Passwords do not match').'</span>'."'"; ?>).slideDown('fast');
		$('#user_password_input').val('');
	...
	{
		$('#new_user_message_p').html(<?php echo "'".'<img src="img/loading.gif" alt="Loading">'. __('Creating user...')."'"; ?>).slideDown('fast');
		...
		$.post('login.php?create_user', { user_name: user_name, user_email: user_email, user_password: user_password, user_secret_code: user_secret_code }, function(data)
		{
			if(data == 1)
			{
				...
				setTimeout(function()
				{
					$('#new_user_message_p').html(<?php echo "'".__('User created successfully! Logging in...').'<img src="img/loading.gif" alt="Loading">'."'"; ?>);
...
function toggle_reservation_time(id, week, day, time, from)
{
	if(session_user_is_admin == '1')
	{
		...
		{
			notify(<?php echo "'".__('You are reserving back in time. You can do that because you are an admin')."'";?>, 4);
		}
		else if(week > global_week_number + global_weeks_forward)
		{
			notify(<?php echo "'".sprintf(__('You are reserving more than %d weeks forward in time. You can do that because you are an admin'),global_weeks_forward)."'"; ?>, 4);
		}
	}
	...
	else
	{
		if(offclick_event == 'mouseup' || from == 'details')
		{
			...
			else if(user_name == session_user_name || session_user_is_admin == '1')
			{
				...
				{
					var delete_confirm = confirm(<?php echo "'".__('This is not your reservation, but because you are an admin you can remove other users reservations. Are you sure you want to do this?')."'"; ?>);
				}
				else
				{
			...
			{
				notify(<?php echo "'".__('You can not remove other users reservations')."'"; ?>, 2);
			}
...
{
	if(typeof id != 'undefined' && $(id).html() != '' && $(id).html() != <?php echo "'".__('Wait...')."'"; ?>)
	{
		if($('#reservation_details_div').is(':hidden'))
		{
			...
			$('#reservation_details_div').html(<?php echo "'".__('Getting details...')."'"; ?>);
			...
			$.post('reservation.php?read_reservation_details', { week: week, day: day, time: time }, function(data)
			{
				setTimeout(function()
				{
					if(data == 0)
					{
						$('#reservation_details_div').html(<?php echo "'".__('This reservation no longer exists. Wait...')."'"; ?>);
						
						setTimeout(function()
						{
							if($('#reservation_details_div').is(':visible') && $('#reservation_details_div').html() == <?php echo "'".__('This reservation no longer exists. Wait...')."'"; ?>)
							{
					...
					else
					{
						...
						if(offclick_event == 'touchend')
						{
							...
							{
								var delete_link_html = <?php echo "'".'<a href="." onclick="toggle_reservation_time(reservation_details_id, reservation_details_week, reservation_details_day, reservation_details_time,'."\'".'details'."\'".'); return false">'.__('Delete').'</a> | '."'"; ?>;
							}
							else
							{
							...
							$('#reservation_details_div').append('<br><br>'+delete_link_html+<?php echo "'".'<a href="." onclick="read_reservation_details(); return false">'.__('Close this').'</a>'."'"; ?>);
						}
					}
				}, 500);
			});
